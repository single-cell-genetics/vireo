...
def match_donor_VCF(cell_dat, donor_vcf):
    ...
    if len(idx1) == 0:
        print(
            "[vireo] warning: no variants matched to donor VCF, "
            + "please check chr format!"
        )
    else:
        print(
            "[vireo] %d out %d variants matched to donor VCF"
    ...
    for _key in cell_dat["FixedINFO"].keys():
        cell_dat["FixedINFO"][_key] = [cell_dat["FixedINFO"][_key][x] for x in idx1]

    donor_vcf["variants"] = [donor_vcf["variants"][x] for x in idx2]
    for _key in donor_vcf["FixedINFO"].keys():
        donor_vcf["FixedINFO"][_key] = [donor_vcf["FixedINFO"][_key][x] for x in idx2]
    for _key in donor_vcf["GenoINFO"].keys():
        donor_vcf["GenoINFO"][_key] = [donor_vcf["GenoINFO"][_key][x] for x in idx2]
...
def read_cellSNP(dir_name, layers=["AD", "DP"]):
    ...
    """
    cell_dat = load_VCF(
        dir_name + "/cellSNP.base.vcf.gz", load_sample=False, biallelic_only=False
    )
...
def write_donor_id(out_dir, donor_names, cell_names, n_vars, res_vireo):
    ...
    donor_doublet = np.array(doublet_names, "U100")[np.argmax(doublet_prob, axis=1)]
