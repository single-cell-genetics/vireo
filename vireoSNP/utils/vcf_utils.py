...
def parse_sample_info(sample_dat, sparse=True, format_list=None):
    ...
    if len(idx_low_tag) > 0:
        print(
            "[vireo] Warning: too few variants with tags!",
            "\t".join(
                [
                    format_list[k] + ": " + str(n_SNP_tagged[k])
                    for k in range(len(format_list))
                ]
            ),
        )
...
def load_VCF(
    vcf_file, biallelic_only=False, load_sample=True, sparse=True, format_list=None
):
    """
    Load whole VCF file
    -------------------
...
def write_VCF_to_hdf5(VCF_dat, out_file):
    ...
    f.create_dataset(
        "contigs",
        data=np.string_(VCF_dat["contigs"]),
        compression="gzip",
        compression_opts=9,
    )
    f.create_dataset(
        "samples",
        data=np.string_(VCF_dat["samples"]),
        compression="gzip",
        compression_opts=9,
    )
    f.create_dataset(
        "variants",
        data=np.string_(VCF_dat["variants"]),
        compression="gzip",
        compression_opts=9,
    )
    f.create_dataset(
        "comments",
        data=np.string_(VCF_dat["comments"]),
        compression="gzip",
        compression_opts=9,
    )
    ...
    for _key in VCF_dat["FixedINFO"]:
        fixed.create_dataset(
            _key,
            data=np.string_(VCF_dat["FixedINFO"][_key]),
            compression="gzip",
            compression_opts=9,
        )
    ...
    for _key in VCF_dat["GenoINFO"]:
        geno.create_dataset(
            _key,
            data=np.string_(VCF_dat["GenoINFO"][_key]),
            compression="gzip",
            compression_opts=9,
        )
...
def parse_donor_GPb(GT_dat, tag="GT", min_prob=0.0):
    ...
    if len(GT_dat) == 0:
        ...
        number_of_var = len(GT_dat[0])

    GT_prob = np.zeros((len(GT_dat), number_of_var, 3))
    for i in range(GT_prob.shape[0]):
...
def match_VCF_samples(VCF_file1, VCF_file2, GT_tag1, GT_tag2):
    ...
    print(
        "n_variants in VCF1, VCF2 and matched: %d, %d, %d"
