...
def main():
    ...
    parser.add_option(
        ...
        default=None,
        help=(
            "The cell genotype file in VCF format or cellSNP folder with "
            "sparse matrices."
        ),
    )
    parser.add_option(
        ...
        default=None,
        help=(
            "Number of donors to demultiplex; can be larger "
    ...
    group0.add_option(
        ...
        default=None,
        help=(
            "The cell genotype files in vartrix outputs (three/four files, "
    ...
    group1.add_option(
        ...
        default=0,
        help=(
            "Number of extra donor in pre-cluster, when GT "
    ...
    group1.add_option(
        ...
        default="distance",
        help=(
            "Method for searching from extra donors. "
    ...
    group1.add_option(
        ...
        default=False,
        action="store_true",
        help=("If use, detect ambient RNAs in each cell " "(under development)"),
    )
    group1.add_option(
        "--nproc",
        "-p",
        ...
        default=1,
        help=(
            "Number of subprocesses for computing - this sacrifices memory "
    ...
    if options.cell_data is None and options.vartrix_data is None:
        ...
        cell_dat = read_vartrix(
            vartrix_files[0], vartrix_files[1], vartrix_files[2], vartrix_files[3]
        )
    ...
    if options.donor_file is not None:
        ...
        if donor_vcf["n_SNP_tagged"][0] < (
            ...
            print(
                "[vireo] No " + options.geno_tag + " tag in donor genotype; "
    ...
    print(
        "[vireo] Demultiplex %d cells to %d donors with %d variants."
        % (cell_dat["AD"].shape[1], n_donor, cell_dat["AD"].shape[0])
    )
    res_vireo = vireo_wrap(
        cell_dat["AD"],
        cell_dat["DP"],
        n_donor=n_donor,
        GT_prior=donor_GPb,
    ...
    if n_donor is not None and donor_GPb is not None and n_donor < donor_GPb.shape[1]:
    ...
    print("[vireo] All done: %d min %.1f sec" % (int(run_time / 60), run_time % 60))
    print()
